...
def my_strategy(symbol, start_date, end_date):
    ...
    df_data['Bol_upper_0.5'] = pd.rolling_mean(df_data[symbol], window=20) + 0.5* pd.rolling_std(df_data[symbol], 20, min_periods=20)
    df_data['Bol_lower'] = pd.rolling_mean(df_data[symbol], window=20) - 2* pd.rolling_std(df_data[symbol], 20, min_periods=20)
    df_data['Bol_lower_0.5'] = pd.rolling_mean(df_data[symbol], window=20) - 0.5* pd.rolling_std(df_data[symbol], 20, min_periods=20)
    df_data['20d_ma_spy'] = pd.rolling_mean(df_data['SPY'], window=20)
    df_data['Bol_upper_spy'] = pd.rolling_mean(df_data['SPY'], window=20) + 2* pd.rolling_std(df_data['SPY'], 20, min_periods=20)
    df_data['Bol_lower_spy'] = pd.rolling_mean(df_data['SPY'], window=20) - 2* pd.rolling_std(df_data['SPY'], 20, min_periods=20)
    df_data_2=df_data.ix[dates_2]
    df_data_2=df_data_2.dropna(axis=0)
    plot=df_data.plot(y=symbol+['20d_ma','Bol_upper','Bol_lower','Bol_upper_0.5','Bol_lower_0.5'],color=['b','y','g','g','r','r'])
    plot.legend(['IBM', 'SMA','Bollinger Bands','Bollinger Bands by 0.5'], loc='best')
    df_data['Sell']=False
    df_data['Buy']=False
    df_data['End']=False
    df_data['portfolio']=0
    for index,date in enumerate(df_data.index):
        if index > 19 and index < len(df_data.index)-1:
            if (df_data.ix[df_data.index[index-1]]['Bol_upper']<df_data.ix[df_data.index[index-1]][symbol[0]]) and (df_data.ix[date]['Bol_upper']>df_data.ix[date][symbol[0]]) and (df_data.ix[date]['20d_ma']<df_data.ix[date][symbol[0]]) and (df_data.ix[df_data.index[index-1],'portfolio']==0):
                ...
                df_data.ix[date.date(),'portfolio']=100
            elif (df_data.ix[df_data.index[index-1]]['portfolio'] > 0 and df_data.ix[date][symbol[0]] > df_data.ix[date]['Bol_upper_0.5']) or (df_data.ix[df_data.index[index-1]]['portfolio'] < 0 and df_data.ix[date][symbol[0]] < df_data.ix[date]['Bol_upper_0.5']):
    ...
    df_order.to_csv(path_or_buf='/Users/chen/Downloads/ml4t/mc2_p2/orders/orders_mc2_p2_my_strategy.csv',index_label='Date')
    return df_order
def test_run():
    """Driver function."""
    ...
    start_val = 10000
    symbol = ["IBM"]
    my_strategy(symbol, start_date,end_date)
    orders_file = os.path.join("orders","orders_mc2_p2_my_strategy.csv")
